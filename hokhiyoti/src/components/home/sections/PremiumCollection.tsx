...
export default function PremiumCollection() {
  ...
  const loadMore = async (isInitial = false) => {
    ...
    try {
      const res = await supabaseProductService.getPremiumCollectionProducts(limit, activeCursor)
      if (res.length > 0) {
        setProducts((prev) => {
      ...
      console.error('Failed to load premium collection products:', err)
    } finally {
      setLoading(false)
    }
  }
  ...
  // Hide section entirely if no premium collection products are tagged and loading has completed
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10">
        <div>
          ...
          <p className="mt-1 font-sans text-xs sm:text-sm text-[#8a8a8a] font-light">
            Handcrafted luxury pieces curated for connoisseurs of fine attire.
          </p>
        </div>
      </div>
      ...
      {loading && products.length === 0 ? (
        ...
        <>
          ...
          <div className="hidden md:grid md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 lg:gap-5">
            {products.map((product) => (
              <ProductCard key={`dt-prem-${product.id}`} product={product} />
            ))}
          </div>
        </>
      )}
