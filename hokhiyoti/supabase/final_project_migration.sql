...
AS $$
  SELECT
    COALESCE(category_name, 'General Curations') AS category_name,
    COUNT(*) AS order_count,
    COALESCE(SUM(product_price), 0)::numeric AS total_revenue
  FROM orders
  WHERE order_status = 'delivered'
  GROUP BY COALESCE(category_name, 'General Curations')
